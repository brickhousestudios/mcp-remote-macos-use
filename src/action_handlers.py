...
# Import connection manager for connection pooling
...
def _get_vnc_manager():
    """
    Get a VNC connection manager with environment settings.

    Returns:
        VNCConnectionManager: Configured connection manager
    """
    return get_vnc_manager(
        host=MACOS_HOST,
        port=MACOS_PORT,
...
def handle_remote_macos_mouse_scroll(arguments: dict[str, Any]) -> list[types.TextContent | types.ImageContent | types.EmbeddedResource]:
    ...
    # Get connection manager and execute operation
    manager = _get_vnc_manager()
